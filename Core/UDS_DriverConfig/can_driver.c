...
void RxCANMsgMainFun(void)
{
    ...
    uint8_t CANDataIndex = 0u;
		...
		HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO0, &hCAN1_RxHeader, aRxData);
		
    stRxCANMsg.usRxDataId = hCAN1_RxHeader.StdId;
    stRxCANMsg.ucRxDataLen = hCAN1_RxHeader.DLC;
            ...
            (TRUE == IsRxCANMsgId(stRxCANMsg.usRxDataId)))
    {
        ...
        {
            stRxCANMsg.aucDataBuf[CANDataIndex] = aRxData[CANDataIndex];
        }
...
HAL_StatusTypeDef CAN_WriteData(CAN_TxHeaderTypeDef *TxMessageHeader, uint8_t *TxMessage)
{
  uint32_t TxMailbox = 0xFF;  
  uint32_t TimeOut=0;
	
	HAL_CAN_AddTxMessage(&hcan1,TxMessageHeader,TxMessage,&TxMailbox);
                       ...
                       const uint32_t i_txBlockingMaxtime)
{
    CAN_TxHeaderTypeDef 	TxMessageHeader;
		...
		TxMessageHeader.DLC = i_ucDataLen;
	
    if(TX_RESP_ADDR_ID_TYPE == CAN_ID_STD)
    {
			TxMessageHeader.IDE = CAN_ID_STD;
			TxMessageHeader.StdId = i_usCANMsgID;
    }
    else
    {
			TxMessageHeader.IDE = CAN_ID_EXT;
			TxMessageHeader.ExtId = i_usCANMsgID;
    }

		CAN_WriteData(&TxMessageHeader,(uint8_t *)i_pucDataBuf);
