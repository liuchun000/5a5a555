...
//    EndPageAddr =  i_ulLogicalAddr + i_ulEraseLen - 1;
//    for(i=i_ulLogicalAddr;i<=EndPageAddr;i+=PAGE_SIZE)
//    {
//        FLASHStatus = g_pstFlashOptInfo->FLASH_EraseSector(i);
...
//    uint32_t i;
//    uint32_t StartAddress = i_xStartAddr;
//    uint8_t *pData = (uint8_t *)i_pvDataBuf;
//    FLASH_Unlock();
//   /* Clear All pending flags */
//    FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR); 

//    for(i=0;i<(i_usDataLen>>2);i++)
//    {
//        FLASHStatus = g_pstFlashOptInfo->FLASH_Program(StartAddress, *((uint32_t*)pData));
//        if (FLASHStatus == FLASH_COMPLETE)
//        {
//            StartAddress += 4;
//            pData += 4;
